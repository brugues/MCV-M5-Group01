...
def print_rect(frame, rect, color, dt_id):
    ...
    if dt_id is not None:
        cv2.putText(frame, str(dt_id), (pt1[0], pt1[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)
...
class MultiTracker():
    def __init__(self, ttype, maxDisappeared=0, key=lambda x:x, get_patch=None,
                 update_obj = lambda old, new: new, pix_tol=100,
                 iou_threshold=0.3,
