...
from custom_trainer import CTrainer
...
PATH_MOTSCHALLENGE = './output_da/MOTSChallenge/' + MODEL_NAME
PATH_KITTI_MOTS = './output_da/KITTI_MOTS/' + MODEL_NAME + DA
KITTI_MOTS_WEIGHTS = PATH_KITTI_MOTS + '/model_final.pth'

TRAIN_PKL = './pickles/train_' + TRAIN_SET + '_dataset_server.pkl'
VAL_PKL = './pickles/validation_' + VAL_SET + '_dataset_server.pkl'
...
def main():
    ...
    MetadataCatalog.get(cfg.DATASETS.TRAIN[0]).set(crop=True, flip=True, saturation=False)
    # Set-up trainer
    if GET_VALIDATION_PLOTS:
